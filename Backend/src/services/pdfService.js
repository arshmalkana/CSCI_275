...
function renderPage2(doc, data) {
  ...
  const cowBreeds = [
    ...
    { code: 'HF_IMPORTED',     label: 'Imp HF'  },
    { code: 'JERSEY_IMPORTED', label: 'Imp JR'  },
    { code: 'HF_SEXED',        label: 'Sexed'   },
  ]
...
function renderPage3(doc, data) {
  ...
  // ── Buffalo AI — 14 cols, uniform width ──
  const bufWs = cw(CONTENT_W - labelW3, 14)
  const bufHeaders = [
    'Murrah AI','Nili-Ravi AI','Murrah Cov','N-R Cov',
    'Total AI','Total Cov',
  ...
  ;[
    ...
    const cr = (n(mu.tested) + n(nr.tested)) > 0
      ? (((n(mu.positive) + n(nr.positive)) / (n(mu.tested) + n(nr.tested))) * 100).toFixed(1) + '%'
      : '0%'
    const vals = [
      n(mu.ai), n(nr.ai), n(mu.covered), n(nr.covered),
      n(mu.ai)+n(nr.ai), n(mu.covered)+n(nr.covered),
